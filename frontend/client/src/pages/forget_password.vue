      ...
      <div class="flex justify-center items-center">
        <div class="p-6 sm:w-[500px] w-full bg-[#FFFFFF] rounded-md shadow-md">
          <div class="flex items-center justify-center">
            <span class="text-sm font-bold text-gray-700">Password Recovery</span>
          </div>
          <Form
          ...
          >
            <div class="space-y-1">
              <div class="space-y-3 w-full">
                <label class="font-bold text-sm text-[#353F52]">Enter Your Email Account</label>
                <Field
                  name="email"
                  type="text"
                  v-model="email"
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (response.data.forget_password.success) {
      loginreturn.value = 'Email Sent Successfully'
      notify({
        type: 'success',
        text: 'Recovery Code Sent to Your Email'
      })
      router.push('/verification-code')
      console.log(response.data.forget_password.success)
      localStorage.setItem('email', email.value)
    } else {
      loginreturn.value = 'Email Not Sent'
      notify({
        type: 'error',
        text: 'Email Not Sent'
      })
