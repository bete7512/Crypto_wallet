      ...
      >
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <div class="space-y-1 text-base">
            <label class="block text-lg text-black"> Service Detail </label>
            <textarea
              v-model="feedback_store.service_detail"
          ...
          <div class="space-y-1 text-base">
            <label class="block text-lg text-black"> Service Quality </label>
            <textarea
              v-model="feedback_store.service_quality"
              class="shadow appearance-none border h-20 text-base bg-[#E5E7EB] rounded w-full py-2 px-3 placeholder-gray-500 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
            ></textarea>
          </div>
          ...
          <div class="space-y-1 text-base">
            ...
            <div class="space-y-1 text-base">
              <label class="block text-lg text-black"> Suggession </label>
              <textarea
                v-model="feedback_store.suggession"
                class="shadow appearance-none border h-20 text-base bg-[#E5E7EB] rounded w-full py-2 px-3 placeholder-gray-500 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
              ></textarea>
            </div>
            <div class="flex space-x-2">
              <div class="flex items-center">
                <input
                  v-model="feedback_store.is_service_completed"
                  type="checkbox"
...
const submitForm = async (event) => {
  feedback_store.service_id = props.service_req_id;
  try {
    let res = await feedback_store.insert_customer_feedback();
    if (typeof res == "string") {
      let data = "Feedback Submitted Successfully";
      emit("success", data);
    } else {
      let data = "Feedback Submission Failed";
      emit("failed", data);
      return;
    }
    return;
