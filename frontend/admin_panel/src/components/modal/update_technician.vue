                ...
                <div class="flex flex-col space-y-2">
                  <label for="verified" class="flex items-center">
                    <input
                      ...
                      class="mr-2"
                    />
                    <span class="text-base text-neutral-600">Verify</span>
                  </label>
                </div>
              </div>
...
const zone = reactive({
  ...
  select_zone: (zo) => {
    console.log("select zone", technician.technician_id);
...
watchEffect(async () => {
  ...
  if (technician.region_id) {
    await address.get_zones(technician.region_id);
  }
...
onMounted(async () => {
  ...
  await technician.get_technician_by_id(props.technician_id);
});
watchEffect(async () => {
  if (technician.region_id) {
    await address.get_zones(technician.region_id);
...
async function onSubmit() {
  ...
  try {
    console.log("props.technician_id", props.technician_id);
    const response = await technician.update_technician(props.technician_id);
