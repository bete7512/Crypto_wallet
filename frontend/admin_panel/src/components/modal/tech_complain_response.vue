<template>
        ...
        >
          ...
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="space-y-1 text-base">
              <label class="block text-lg  text-black"> Response </label>
              <textarea
                v-model="technician_complain_store.response"
  ...
  import { technicianComplainStore } from "../../stores/tech_complain_store";
  ...
  const technician_complain_store = technicianComplainStore();
  
  const submitForm = async () => {
    technician_complain_store.id = props.id;
    try {
      const response = await technician_complain_store.insert_technician_response_complain();
