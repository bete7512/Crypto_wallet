<template>
  <div class="w-full h-1/2 2xl:w-[48%]">
    <div class="p-2 space-y-1 w-full bg-white rounded-lg shadow">
      <h2 class="text-xl font-bold mb-4 text-gray-400">Regions</h2>
        ...
        <table class="table-fixed w-full sm:px-2 text-gray-700">
          ...
          <tbody class="text-sm font-normal">
            ...
            >
              ...
              <td class="py-3 px-1 text-left"> {{ region.name }}</td>
              <td class="py-3 px-1 text-left line-clamp-1">
                {{ region?.description ?? "No description" }}
              </td>
              <td class="py-3 px-1 text-left">{{ region.created_at.split('T')[0] }}</td>
...
query MyQuery($limit: Int = 6, $offset: Int = 0, $search: String = "%%") {
  region(limit: $limit, offset: $offset, where: {name: {_ilike: $search}}) {
    created_at
    id
    name
  }
...
const regions = computed(()=>result.value?.region ?? [])
