<template>
    <div class="w-full h-1/2 mx-5">
   <div class="p-2 space-y-1 w-full bg-white rounded-lg shadow">
     <h2 class="text-xl font-bold mb-4 text-gray-400">Subscription Plan</h2>
       ...
       <table class="table-fixed w-full sm:px-2 text-gray-700">
         <thead class="bg-gray-100 text-xs uppercase font-medium">
           <tr>
             ...
             <th class="py-3 px-1 text-left"> Status</th>
             <th class="py-3 px-1 text-left"> Price</th>
             <th class="py-3 px-1 text-left line-clamp-1"> Number of service</th>
         ...
         <tbody class="text-sm font-normal">
           <tr
             v-for="(plan,i) in subscription_plans"
             :key="plan.id"
             class="bg-white border-b hover:bg-gray-100"
           >
             <td class="py-3 px-1 text-left">{{ i + 1 }}</td>
             <td class="py-3 px-1 text-left">{{plan.name}}</td>
             <td class="py-3 px-1 text-left ">{{ plan.is_active ? "Yes" : "No" }}</td>
             <td class="py-3 px-1 text-left ">{{ plan.price }}</td>
             <td class="py-3 px-1 text-left ">{{ plan.number_of_service }}</td>
             <td class="py-3 px-1 text-left">{{ plan.created_at.split('T')[0] }}</td>
...
query MyQuery($limit: Int = 10, $offset: Int = 0, $search: String = "%%") {
  subscription_plan(limit: $limit, offset: $offset, where: {name: {_ilike: $search}}) {
    created_at
    id
    name
    updated_at
...
const subscription_plans = computed(()=>result.value?.subscription_plan ?? [])
</script>
<style >
