<template>
  <div class="w-full h-1/2 mx-5 ">
    <div class="p-2 space-y-1 w-full bg-white rounded-lg shadow">
      <h2 class="text-xl font-bold mb-4 text-gray-400">Service Classes</h2>
        ...
        <table class="table-fixed w-full sm:px-2 text-gray-700">
          <thead class="bg-gray-100 text-xs uppercase font-medium">
            <tr>
              ...
              <th class="py-3 px-1 text-left">Problem Name</th>
              <th class="py-3 px-1 text-left line-clamp-1">
                Problem Description
              </th>
              <th class="py-3 px-1 text-left">Date</th>
              <th class="py-3 px-1">Actions</th>
            </tr>
          </thead>
          <tbody class="text-sm font-normal">
            <tr
              v-for="(service, i) in service_classes"
              :key="service.id"
              class="bg-white border-b hover:bg-gray-100"
            >
              <td class="py-3 px-1 text-left">{{ i + 1 }}</td>
              <td class="py-3 px-1 text-left">{{ service.name }}</td>
              <td class="py-3 px-1 text-left line-clamp-1">
                {{ service.description ?? "Have not description" }}
              </td>
...
const service_classes = computed(() => result.value?.problem_class ?? []);
</script>
<style></style>
