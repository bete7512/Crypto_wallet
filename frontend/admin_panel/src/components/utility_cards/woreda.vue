<template>
    <div class="w-full h-1/2 2xl:w-[48%]">
   <div class="p-2 space-y-1 w-full bg-white rounded-lg shadow">
     <h2 class="text-xl font-bold mb-4 text-gray-400">Woredas</h2>
     ...
     <div class="overflow-x-scroll w-full">
       <table class="table-fixed w-full sm:px-2 text-gray-700">
         ...
         <tbody class="text-sm font-normal">
           <tr
             v-for="(woreda,i) in woredas"
             :key="woreda.id"
             class="bg-white border-b hover:bg-gray-100"
           >
             <td class="py-3 px-1 text-left">{{ i + 1 }}</td>
             <td class="py-3 px-1 text-left">{{woreda.name}}</td>
             <td class="py-3 px-1 text-left line-clamp-1">{{ woreda?.description ?? "no description" }}</td>
             <td class="py-3 px-1 text-left">{{ woreda.created_at.split('T')[0] }}</td>
...
query MyQuery($limit: Int = 6, $offset: Int = 0, $search: String = "%%") {
  woreda(limit: $limit, offset: $offset, where: {name: {_ilike: $search}}) {
    created_at
    id
    name
  }
...
const woredas = computed(()=>result.value?.woreda ?? [])
