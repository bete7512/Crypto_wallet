                    ...
                    <div class="mt-4 mx-2" v-if="order.order_items.length > 0">
                      ...
                      >
                        <div class="absolute mx-2 top-2 text-xl right-0 flex">
                          <input v-model="tempo_order_items[i].is_selected" class="h-4 w-4" value="true" type="checkbox" />
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                          <input value="true" v-model="tempo_order_items[i].is_warranty" class="h-4 w-4" type="checkbox" />
                          <p
                            :class="[
                              warranty_end_date(
...
const region = reactive({
  ...
  add_new_region: async () => {
    ...
    if (typeof hold == "string") {
     ...
     region.select_region({ id: hold, name: new_region.value });
     service_request.region_id = hold;   
     alert(service_request.region_id)          
    region.show_add_region_option = false;        
  },
});
...
const problem = reactive({
  ...
  add_new_problem: async () => {
    ...
    problem.select_problem({ id: hold, name: new_problem.value })
...
const zone = reactive({
  ...
  add_new_zone: async () => {
    ...
    if (typeof hold == "string") {
      ...
      zone.select_zone({ id: hold, name: new_zone.value });             
...
const woreda = reactive({
  ...
  add_new_woreda: async () => {
    ...
    if (typeof hold == "string") {
      ...
      woreda.select_woreda({ id: hold, name: new_woreda.value });          
...
watchEffect(
  () => {
    console.log("tempo_order_items changed");
    const selectedOrderItem = tempo_order_items.value.find((item) => item.is_selected == true)
    console.log(selectedOrderItem);
    if (selectedOrderItem) {
      service_request.product_id = selectedOrderItem.product_id;
      service_request.order_item_id = selectedOrderItem.order_id;
      service_request.is_in_warranty_request = selectedOrderItem.is_warranty;  
    }
  },
...
async function fetch_customer_data() {
  ...
  try {
    ...
    console.log("It was very amazing" + response);
    user_id.value = response.id;
    console.log("It was very amazing" + user_id.value);
    ...
    if (user_id.value) {
      try {
        ...
        //   order.value.order_items = data.map(item => ({ ...item, checked: false }));
...
async function onSubmit() {
  ...
  try {
    ...
    const response = await service_request.insert_service_request();
